...
function readQuote(day) {
    db.collection("quotes").doc(day)                                                      //name of the collection and documents should match exactly with what you have in Firestore
    ...
    db.collection(collection)
        ...
        .then(allHikes => {
            ...
            allHikes.forEach(doc => { //iterate thru each doc
                ...
                // Demo 11 Step 2.1.3
                newcard.querySelector('i').id = 'save-' + docID;     //guaranteed to be unique - assign unique id to each element
                newcard.querySelector('i').onclick = () => saveBookmark(docID);
                // If we do not have the () callback, it will just execute the saveBookmark instantly, we need an event listener
...
//         // let = isBookmarked = bookmarks.includes(hikeID) // check if this hikeID exist in bookmark array
//         currentUser.update({
...
//         })
        ...
        //             bookmarks: firebase.firestore.FieldValue.arrayUnion(hikeID)
        //         }).then(() => {
        //             console.log("item added to database" + hikeID)
